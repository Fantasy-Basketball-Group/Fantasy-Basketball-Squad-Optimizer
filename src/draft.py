...
class draft:
    def __init__(
        self, league, num_of_rounds, player_pool, draft_order, already_drafted
    ):
        self.league = league
    ...
    while pick <= total_picks:
        ...
        if choice == "1":  # Draft Player
            ...
            draft.league.map_of_league[on_clock].roster = sfb.draft_player(
                player_name,
                draft.league.map_of_league[on_clock].roster,
                draft.player_pool,
            )
            draft.already_drafted[pick] = player_name
            print("#################################################################")
            print(
                "With the", pick, "pick in the draft,", on_clock, "selects", player_name
            )
            print("#################################################################")
            pick += 1
            ...
            print(team_owner, draft.league.map_of_league[team_owner].roster.to_string())
        elif choice == "5":  # Optimizer Analysis
            print(
                optimizer_analysis(
                    draft.league.map_of_league[on_clock].roster, draft.player_pool
                ).to_string()
            )
        elif choice == "6":  # Quit
            break
...
already_drafted = {}
draft = draft(my_league, num_of_rounds, player_pool, draft_order, already_drafted)
